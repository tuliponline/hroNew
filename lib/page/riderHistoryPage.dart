...
class RiderHistoryPagr extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return RiderHistoryState();
  }
}

class RiderHistoryState extends State<RiderHistoryPagr> {
  FirebaseFirestore db = FirebaseFirestore.instance;
  List<RiderHistoryListModel> riderHistoryListModel;

  int costDeliveryAll = 0;
  int amountAll = 0;
  int orderCount = 0;
  ...
  _getDriverHistory(AppDataModel appDataModel) async {
    ...
    await db
        .collection("orders")
        .where("driver", isEqualTo: appDataModel.userOneModel.uid)
      ...
      riderHistoryListModel.forEach((e) {
        ...
        if (timeStamp.isAfter(startDate) && timeStamp.isBefore(stopDate)) {
          int cd = int.parse(e.costDelivery);
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                Style()
                                    .textSizeColor("ค่าส่ง", 14, Colors.black),
                                Style().textSizeColor(
                                    appDataModel.moneyFormat
                                            .format(costDeliveryAll) +
                                        " ฿",
                                    14,
                                    Colors.black)
                              ],
